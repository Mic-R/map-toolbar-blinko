...
export function App({
  ...
  onClick?: (locationInfo: PlaceInfo | null) => void;
  style?: CSSProperties;
}): JSXInternal.Element {
  ...
  const getPlaceNameFromCoords = async (
    lat: number,
    lng: number
  ): Promise<string> => {
    try {
      ...
      return i18n.t("map.locationPoint", {
        lat: lat.toFixed(4),
        lng: lng.toFixed(4),
      });
    } finally {
